...
from factory import PresentationError, MetadataError, ConfigurationError, \
    StructuralError, RequirementError, DataError
import StringIO
import json
...
class ManifestReader(object):
    ...
    def getVersion(self, js):
        if not js.has_key('@context'):
            raise SerializationError('Top level resource MUST have @context',
                                     js)
        ctx = js['@context']
        try:
        ...
        if self.require_version and self.require_version != version:
            raise SerializationError(
                'Expected version %s context, got version %s' % (
                    self.require_version, version))
    ...
    def read(self):
        ...
        if jsonld:
            try:
                ...
                raise SerializationError("Data is not valid JSON-LD: %r" % e,
                                         data)
        return top
    ...
    def readObject(self, js, parent=None, parentProperty=None):
        ...
        if hasattr(parent, fn):
            ...
            raise StructuralError(
                "Unknown resource class " + typ + " from parent: " + parent._type,
                parent)
        ...
        for req in what._required:
            if not js.has_key(req):
                if what._structure_properties.has_key(req):
                    ...
                    if parentProperty and parent._structure_properties.get(
                            parentProperty, {}).get('minimal', False):
                        continue
                    if req == 'canvases' and len(parent.sequences) > 1:
                        # Do not Allow second and future sequences if not minimal
                        continue
                    raise StructuralError(
                        "%s['%s'] not present and required" % (what._type, req),
                        what)
                else:
                ...
                raise StructuralError("Second Sequence must not list canvases",
                                      what)
        ...
        for (k, v) in js.items():
            ...
            if what._structure_properties.has_key(k):
                if type(v) == list:
                    for sub in v:
                        ...
                        elif type(sub) in [str, unicode] and sub.startswith(
                                'http'):
                    ...
                    raise StructuralError(
                        "%s['%s'] must be a list, got: %s" % (what._type, k, v),
                        what)
                elif type(v) == dict:
                    subo = self.readObject(v, what, k)
                elif type(v) in [str, unicode] and (
                                v.startswith('http') or v.startswith(
                                    'urn:') or v.startswith('_:')):
                    setattr(what, k, v)
                else:
                    raise StructuralError(
                        "%s['%s'] has broken value: %r" % (what._type, k, v),
                        what)
            ...
            # Process descriptive fields
                ...
                if self.require_version and self.require_version != "0.9":
                    raise RequirementError(
                        "Old property from 0.9 seen: %s expected version %s" % (
                            k, self.require_version))
                pass
            elif k == "resources":
