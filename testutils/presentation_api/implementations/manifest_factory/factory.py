...
VIEWINGDIRS = ['left-to-right', 'right-to-left', 'top-to-bottom',
               'bottom-to-top']

BAD_HTML_TAGS = ['script', 'style', 'object', 'form', 'input']
GOOD_HTML_TAGS = ['a', 'b', 'br', 'i', 'img', 'p', 'span']

KEY_ORDER = ["@context", "@id", "@type", "@value", "@language", "label",
             "value",
             "metadata", "description", "thumbnail", "attribution", "license",
             "logo",
             "format", "height", "width", "startCanvas",
             "viewingDirection", "viewingHint",
             "profile", "seeAlso", "search", "formats", "qualities", "supports",
             "scale_factors", "scaleFactors", "tile_width", "tile_height",
             "tiles", "sizes",
             "within", "motivation", "stylesheet", "resource",
             "on", "default", "item", "style", "full", "selector", "chars",
             "language",
             "sequences", "structures", "canvases", "resources", "images",
             "otherContent"]
...
class ManifestFactory(object):
    ...
    def __init__(self, version="2.0", mdbase="", imgbase="", mddir="",
                 lang="en"):
    ...
    def set_base_metadata_dir(self, dir):
        if not os.path.exists(dir):
            raise ConfigurationError(
                "Metadata API Base Directory does not exist")
        elif dir[-1] != "/":
            dir += "/"
...
class BaseMetadataObject(object):
    _properties = ['id', 'type', 'label', 'metadata', 'description',
                   'thumbnail',
                   'attribution', 'license', 'logo', 'service', 'seeAlso',
                   'within', 'related',
                   'viewingHint', 'viewingDirection']
    _extra_properties = []
    _integer_properties = []
    _structure_properties = {}
    _object_properties = ['thumbnail', 'license', 'logo', 'seeAlso', 'within',
                          'related', 'service']
    ...
    def __setattr__(self, which, value):
        if which == 'context':
            ...
            0] != "_" and not which in self._properties and not which in self._extra_properties and not which in self._structure_properties.keys():
            self.maybe_warn(
                "Setting non-standard field '%s' on resource of type '%s'" % (
                    which, self._type))
        elif which[0] != '_' and not type(value) in [str, unicode, list,
                                                     dict] and not which in self._integer_properties and \
                not isinstance(value, BaseMetadataObject) and not isinstance(
                value, OrderedDict):
            ...
            raise DataError("%s['%s'] does not accept a %s, only an integer" % (
                self._type, which, type(value).__name__), self)
        elif value and which in self._object_properties and not self.test_object(
                value):
        ...
        if etree:
            ...
            for elm in dom.iter():
                if elm.tag in BAD_HTML_TAGS:
                    raise DataError(
                        "HTML vulnerability '%s' in '%s'" % (elm.tag, data),
                        self)
                elif elm.tag in [etree.Comment, etree.ProcessingInstruction]:
                    raise DataError("HTML Comment vulnerability '%s'" % elm,
                                    self)
                elif elm.tag == 'a':
                    ...
                    for x in elm.attrib.keys():
                        if not x in ['src', 'alt']:
                            raise DataError(
                                "Vulnerable attribute '%s' on img tag" % x,
                                self)
                else:
                    if elm.attrib:
                        raise DataError(
                            "Attributes not allowed on %s tag" % (elm.tag),
                            self)
    ...
    def langhash_to_jsonld(self, lh, html=True):
        ...
        for (k, v) in lh.items():
            if 'html' in k or (v[0] == '<' and v[-1] == '>'):
                ...
                if v[0] != '<' or v[-1] != '>':
                    raise DataError(
                        "First and last characters of HTML value must be '<' and '>' respectively, in '%r'" % v,
                        self)
                self.test_html(v)
                if k:
            ...
            if not d.has_key(e):
                if self._structure_properties.has_key(e):
                    raise StructuralError(
                        "Resource type '%s' requires '%s' to be set" % (
                            self._type, e), self)
                else:
                    raise RequirementError(
                        "Resource type '%s' requires '%s' to be set" % (
            ...
            if hasattr(self, '_viewing_hints'):
                if not d['viewingHint'] in self._viewing_hints:
                    msg = "'%s' not a known viewing hint for type '%s': %s" % (
                        d['viewingHint'], self._type,
                        ' '.join(self._viewing_hints))
                    self.maybe_warn(msg)
            else:
            ...
            if hasattr(self, '_viewing_directions'):
                if not d['viewingDirection'] in self._viewing_directions:
                    msg = "'%s' not a known viewing direction for type '%s': %s" % (
                        d['viewingDirection'], self._type,
                        ' '.join(self._viewing_directions))
            ...
            if d.has_key(p):
                if type(d[p]) == list:
                    ...
                    if sinfo.get('list', False):
                        raise StructuralError(
                            "%s['%s] must be a list, got %r" % (
                                self._type, p, d[p]), self)
                    d[p] = self._single_toJSON(d[p], sinfo, p)

        return OrderedDict(
            sorted(d.items(), key=lambda x: KEY_ORDER_HASH.get(x[0], 1000)))
    ...
    def _single_toJSON(self, instance, sinfo, prop, minimalOveride=False):
        ...
        if type(instance) in [str, unicode]:
            # Just a URI
            return instance
        elif ( isinstance(instance, BaseMetadataObject) and typ == None ) or (
                        typ != None and isinstance(instance, typ)):
            if minimal:
                return {'@id': instance.id, '@type': instance._type,
                        'label': instance.label}
            else:
                return instance.toJSON(False)
        elif type(instance) == dict and ((instance.has_key('@type') and
                                                  instance[
                                                      '@type'] == typ._type) or typ == None ):
            if minimal:
            ...
            raise StructuralError(
                "%s['%s'] objects must be of type %s, got %s" % (
                    self._type, prop, typ._type, instance.get('@type', None)),
                self)
...
class ContentResource(BaseMetadataObject):
    def make_selection(self, selector, summarize=False):
        ...
        if type(selector) == str:
            ...
            selector = OrderedDict(sorted(selector.items(),
                key=lambda x: KEY_ORDER_HASH.get(x[0], 1000)))
    ...
    def add_sequence(self, seq):
        ...
        if seq.id:
            for s in self.sequences:
                if s.id == seq.id:
                    raise DataError(
                        "Cannot have two Sequences with the same identity",
                        self)
...
class Canvas(ContentResource):
    ...
    def toJSON(self, top=False):
        ...
        for anno in self.images:
            ...
            if not (isinstance(res, Choice) or isinstance(res, Image) or (
                        isinstance(res, SpecificResource) and isinstance(
                            res.full,
                            Image))):
                raise StructuralError(
                    "Annotations in Canvas['images'] must have Images for their resources, got: %r" % res,
                    self)
...
for c in [Collection, Manifest, Sequence, Canvas, Range, Layer, Image,
          AnnotationList, Annotation, Service]:
