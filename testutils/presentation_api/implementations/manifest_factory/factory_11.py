...
class ManifestFactory(object):
    ...
    def text(self, txt="", ident="", language="", format=""):
        if not ident and not txt:
            raise ConfigurationError(
                "Text must have either a URI or embedded text")
        elif txt:
...
class BaseMetadataObject(object):
    ...
    def toJSON(self, top=False):
        ...
        if self._factory.debug_level == "warn":
            for e in self._warn:
                if not d.has_key(e):
                    print "WARNING: Resource type '%s' should have '%s' set" % (
                        self._type, e)
        if top:
        ...
        if d.has_key('viewingHint') and not d['viewingHint'] in VIEWINGHINTS:
            raise MetadataError(
                "ViewingHint must be one of: individuals, paged, continuous")
        if d.has_key('viewingDirection') and not d[
            'viewingDirection'] in VIEWINGDIRS:
            raise MetadataError(
                "ViewingDirection must be one of: left-to-right, right-to-left, top-to-bottom, bottom-to-top")
    ...
    def add_range(self, rng):
        ...
        if rng.id:
            for r in self.structures:
                if r.id == rng.id:
                    raise MetadataError(
                        "Cannot have two Ranges with the same identity")
    ...
    def add_canvas(self, cvs):
        if cvs.id:
            for c in self.canvases:
                if c.id == cvs.id:
                    raise MetadataError(
                        "Cannot have two Canvases with the same identity")
...
class Annotation(BaseMetadataObject):
    ...
    def stylesheet(self, css, cls):
        ...
        ss = {"@type": ["oa:CssStyle", "cnt:ContentAsText"],
...
class Choice(BaseMetadataObject):
    ...
    def __init__(self, factory, default, rest):
        ...
        if type(rest) != list:
            rest = [rest]
        self.item = rest
        return super(Choice, self).__init__(factory, indent="", label="",
                                            mdhash={})
...
if __name__ == "__main__":
    ...
    for x in range(2):
        ...
        img.set_hw_from_file(
            "/Users/azaroth/Dropbox/SharedCanvasData/m804/images/f1r.c.jpg")
        img2 = factory.image("f1r", iiif=True)
        img2.set_hw_from_file(
            "/Users/azaroth/Dropbox/SharedCanvasData/m804/images/f1r.jpg")
