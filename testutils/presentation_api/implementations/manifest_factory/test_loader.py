from factory import DataError, StructuralError, RequirementError, \
...
def do_test(data, failmsg, excexp):
    ...
    if excexp == None:
        ...
        try:
            ...
            if debug_tests:
                print "%s caused by %r for test: '%s'" % (
                    e.__class__.__name__, e.resource, failmsg)
            print_warnings(reader)
            return 1
        except:
            raise
...
errorList = [SerializationError, SerializationError, SerializationError,
             SerializationError,
             SerializationError, RequirementError, StructuralError,
             ConfigurationError, ConfigurationError,
             RequirementError, DataError, StructuralError, StructuralError,
             StructuralError, StructuralError,
             StructuralError, StructuralError, StructuralError, StructuralError,
             StructuralError, StructuralError,
             StructuralError, RequirementError, ConfigurationError,
             RequirementError, DataError, RequirementError,
             DataError, RequirementError, DataError, StructuralError,
             StructuralError, StructuralError,
             RequirementError, DataError, StructuralError, StructuralError,
             StructuralError, StructuralError,
             RequirementError, StructuralError, RequirementError,
             ConfigurationError, DataError, DataError,
             DataError, DataError, DataError, DataError, DataError, DataError,
             RequirementError]

errorMap = {}
for x in range(len(errorList)):
    errorMap[
        "http://iiif.io/api/presentation/2.0/example/errors/%s/manifest.json" % x] = \
        errorList[x]
